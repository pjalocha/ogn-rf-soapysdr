...
{ public:
  ...
  ~MonoImage()
   ...
   int NewBlank(int NewWidth, int NewHeight, Type Level=0)
      { if(New(NewWidth, NewHeight)<=0) return 0;
        Blank(Level); return Size; }

   int Extend(int NewWidth, int NewHeight, Type Level=0)
   { int NewSize=NewWidth*NewHeight;
     if(Allocated<NewSize) Reallocate(NewSize);
     if(Allocated<NewSize) { Width=0; Height=0; Size=0; return 0; }
     Size=NewSize;
     ...
     { for( int Line=Height-1; Line>=0; Line--)
       { Type *Old = Data + Line*Width;
         Type *New = Data + Line*NewWidth;
         memcpy(New, Old, Width*sizeof(Type));
         for(int Row=Width; Row<NewWidth; Row++)
         { New[Row] = Level; }
       }
       Width=NewWidth; }
     for( int Line=Height; Line<NewHeight; Line++)
     { Type *Pixel = Data + Line*Width;
...
// -----------------------------------------------------------------------------
   ...
   Type *getRow(int Y) { return Data + Y*Width; }
...
// -----------------------------------------------------------------------------
   ...
   int WriteData(FILE *File) const
   { return fwrite(Data, sizeof(Type), Size, File); }

   int ReadData(FILE *File)
   { return fread (Data, sizeof(Type), Size, File); }

   int WritePGM_8bpp(const char *FileName, Type LogRef=0, Type Scale=1, Type Bias=0, bool VertFlip=0) const
     { FILE *File=fopen(FileName,"wb"); if(File==0) return -1;
       fprintf(File,"P5\n%d %d 255\n", Width, Height);
       uint8_t Line[Width];
       for(int Row=0; Row<Height; Row++)
       { Type *Img = Data;
         if(VertFlip) Img += (Height-1-Row)*Width;
                 else Img += Row*Width;
         for(int Col=0; Col<Width; Col++)
         { Type Pixel=(*Img++);
           if(LogRef)
   ...
   int WriteJPG_8bpp(int Fd, int Quality=80, Type LogRef=0, Type Scale=1, Type Bias=0, bool VertFlip=0) const
     { FILE *File=fdopen(Fd, "wb"); if(File==0) return -1;
       int Size=WriteJPG_8bpp(File, Quality, LogRef, Scale, Bias, VertFlip);
       fclose(File); return Size; }

   int WriteJPG_8bpp(const char *FileName, int Quality=80, Type LogRef=0, Type Scale=1, Type Bias=0, bool VertFlip=0) const
     { FILE *File=fopen(FileName, "wb"); if(File==0) return -1;
       int Size=WriteJPG_8bpp(File, Quality, LogRef, Scale, Bias, VertFlip);
       fclose(File); return Size; }

   int WriteJPG_8bpp(FILE *File, int Quality=80, Type LogRef=0, Type Scale=1, Type Bias=0, bool VertFlip=0) const
     {
